...
MODERATOR_SYSTEM = """You are a sharp, neutral moderator running a high-stakes market debate.
...
class ModeratorAgent:
    ...
    def open_debate(self, debate: dict, expert_names: list[str]) -> str:
        ...
        prompt = f"""Open this debate for: {participants}

Topic: {debate['topic']}
Opening question: {debate['opening_question']}{context_block}

Write a brief (2-3 sentence) framing, then pose the opening question directly."""
    ...
    def synthesize_round(self, history: list[dict], round_num: int) -> str:
        ...
        transcript = "\n\n".join(
            f"{e['speaker']}: {(e['content'] or '')[:280]}" for e in expert_exchanges
        )
        prompt = f"""Round {round_num + 1} transcript:

{transcript}

Identify the 2-3 sharpest points of genuine disagreement. Name the people on each side. Keep to 150 words."""
        return self._call(prompt)
    ...
    def generate_followup(self, debate: dict, history: list[dict], round_num: int) -> str:
        ...
        transcript = "\n\n".join(f"{e['speaker']}: {e['content']}" for e in history[-6:])
        prompt = f"""Based on this debate excerpt, generate one sharp follow-up question:
...
Write only the question."""
    ...
    def final_synthesis(self, history: list[dict]) -> str:
        ...
        transcript = "\n\n".join(
            f"{e['speaker']}: {(e['content'] or '')[:250]}" for e in sampled
        )
...
1. Where is there genuine agreement?
2. Where is disagreement irreconcilable and why?
3. The single most important open question this leaves unresolved."""
        return self._call(prompt)
